<template>
    <div v-loading="pageLoading" element-loading-text="Loading..." class="pan-share-content">
        <div class="pan-share-header-content">
            <div class="pan-share-header-content-wrapper">
                <div class="pan-share-header-title-font-content">
                    <span class="pan-share-header-title-font" @click="goHome">Drive Harbor</span>
                </div>
            ...
            <div class="pan-share-list-wrapper">
                <el-card shadow="always" class="pan-share-list-card">
                        ...
                        <div class="pan-share-list-card-header-share-info-content">
                            ...
                            <div class="pan-share-list-card-header-time">
                                <span class="pan-share-list-card-header-create-date">
                                    <i class="fa fa-clock-o"/>Share time: {{ shareDate }}
                                </span>
                                <span class="pan-share-list-card-header-expire-date">
                                     <i class="fa fa-clock-o"/>Expiration time: {{ shareExpireDate }}</span>
                            </div>
                        </div>
                    ...
                    <div v-if="shareCancelFlag" class="pan-share-list-card-error-message">
                        <span>Sorry, the share has expired or has been cancelled</span>
                    </div>
                ...
                <div class="pan-share-code-wrapper">
                    <el-card :header="shareCodeHeader" shadow="always" class="pan-share-code-card">
                        <el-form
                            ...
                            :model="shareCodeForm"
                            @submit.native.prevent>
                            <el-form-item label="Code" prop="shareCode">
                                <el-input type="text"
                                          ref="shareCodeEl"
...
const checkUsername = (rule, value, callback) => {
        if (!panUtil.checkUsername(value)) {
            callback('Please enter username, 6-16 digits, numbers/letters only')
            return
        }
        callback()
    },
    checkPassword = (rule, value, callback) => {
        if (!panUtil.checkPassword(value)) {
            callback('Please enter password, 8-16 digits')
            return
        }
        callback()
    }
...
const shareCodeFormRules = reactive({
    shareCode: [
        {required: true, message: 'Please enter share code', trigger: 'blur'}
    ]
})
...
const refreshShareInfo = (data) => {
    ...
    shareCodeHeader.value = 'Share: ' + shareName + ", from " + username
    shareDate.value = data.createTime
    if (data.shareDay === 0) {
        shareExpireDate.value = 'Permanent'
    } else {
...
const openShareCodePage = () => {
    shareService.getSimpleShareDetail({
        ...
        if (res.code === 0) {
            ...
            shareCodeHeader.value = 'Share: ' + res.data.shareName + ', from: ' + res.data.shareUserInfoVO.username
        } else {
...
const exit = () => {
    ElMessageBox.confirm('Logout?', 'Reminder', {
...
const downloadFile = () => {
    if (!multipleSelection.value || multipleSelection.value.length === 0) {
        ElMessage.error('Please select the files you want to download')
        return
    }
    for (let i = 0, iLength = multipleSelection.value.length; i < iLength; i++) {
        if (multipleSelection.value[i].folderFlag === 1) {
            ElMessage.error('Folder cannot be downloaded')
            return
        }
    }
...
const doChoseTreeNodeCallBack = () => {
    ...
    if (!checkNode) {
        ElMessage.error('Please select the target folder')
        loading.value = false
        return
    }
...
const saveFiles = (newItem) => {
    if (newItem) {
        ...
        ElMessage.error('Please select the files you want to save')
        return
    }
    userService.infoWithoutPageJump(res => {
        if (res.code === 0) {
