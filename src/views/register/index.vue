    ...
    <div class="content">
        <div class="sub-cont">
            ...
            <div class="form sign-up">
                ...
                <el-button type="warning" class="submit" :loading="loading" @click="doRegister" round>Signup</el-button>
            </div>
        </div>
    </div>
</template>
...
const doRegister = () => {
    if (checkRegisterForm()) {
        ...
        userService.register(registerForm, () => {
            ElMessage.success('Signup success, jumping to the login page...')
...
const checkRegisterForm = () => {
    if (!panUtil.checkUsername(registerForm.username)) {
        ElMessage.error('Please enter the username with 6-16 digits, letters/numbers only')
        return false
    }
    if (!panUtil.checkPassword(registerForm.password)) {
        ElMessage.error('Please enter the password, 8-16 digits')
        return false
    }
    if (!registerForm.rePassword || registerForm.password !== registerForm.rePassword) {
        ElMessage.error('Inconsistent password')
        return false
    }
    ...
    if (!registerForm.answer) {
        ElMessage.error('Please enter the answer')
        return false
    }
    return true
}
