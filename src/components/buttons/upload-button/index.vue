                ...
                <div class="upload-content" id="upload-content">
                    <div class="drag-content">
                        ...
                        <div class="drag-text-content">
                            <el-link :underline="false" type="info">Please Drag the Files, or </el-link>
...
const rebindUploader = () => {
    if (uploader && !uploader.support) {
        ElMessage.error('The browser does not support simple-uploader, please try a different browser')
    }
    ...
    try {
        files.forEach((f) => {
            ...
            if (f.size > panUtil.getMaxFileSize()) {
                throw new Error('The file ' + f.name + 'oversized (limit: ' + panUtil.translateFileSize(panUtil.getMaxFileSize()) + ')')
            }
            let taskItem = {
                target: f,
                filename: f.name,
            ...
            MD5(f.file, (e, md5) => {
                ...
                fileService.secUpload({
                    ...
                    if (res.code === 0) {
                        ElMessage.success('File ' + f.name + 'Upload Success')
                        f.cancel()
...
const doMerge = (file) => {
    ...
    fileService.merge({
        ...
        ElMessage.success('File: ' + file.name + ' Upload Success')
